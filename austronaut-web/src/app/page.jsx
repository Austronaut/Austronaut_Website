...
export default function Home() {
  ...
  const cards = [
    ...
    },
    {
      ...
      description: "Intending to conduct pioneering research on space weather",
      icon: (
        <svg
          className="w-6 h-6 text-gray-800 dark:text-white"
          aria-hidden="true"
      ...
      <div className="w-full flex justify-center mt-30 mb-10 bg-black">
        <div className="flex flex-row rounded-lg overflow-hidden w-full max-w-6xl gap-10">
          <div className="w-full bg-black text-white">
            <h2 className="text-5xl md:text-8xl text-center mb-12 font-inter-bold">
              WE ARE THE AUS<span className="font-inter-regular">TRONAUTS</span>
            </h2>
            <div className="flex flex-wrap justify-center gap-6">
              {cards.map((card, index) => (
                ...
                >
                  <SpotlightCard
                    key={index}
                    className="p-6 rounded-2xl border-white"
                    spotlightColor="rgba(255, 191, 0, 0.2)"
                  >
                    <div className="flex justify-center mb-4">{card.icon}</div>
                    <h3 className="text-2xl font-inter-bold text-center mb-4">
                      {card.title}
                    </h3>
                    <p className="text-base font-inter-italic text-center text-gray-300">
                      {card.description}
                    </p>
                  </SpotlightCard>
                </div>
